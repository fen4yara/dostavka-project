    ...
    public class TelegramService
    {
        ...
        private async Task HandleUpdateAsync(ITelegramBotClient botClient, Update update, CancellationToken cancellationToken)
        {
            ...
            if (messageText == "/start")
            {
                ...
                var user = ConnectionClass.connect.Users.FirstOrDefault(log => log.Email == username);
                if (user != null)
                {
                    user.Phone = Convert.ToInt32(chatId);
                    ConnectionClass.connect.SaveChanges();
                }
                else
                {
                    Console.WriteLine($"Пользователь с именем {username} не найден в базе данных.");
                    await botClient.SendTextMessageAsync(
                        chatId: chatId,
                        text: "Вашего пользователя не найдено в базе данных.",
                        cancellationToken: cancellationToken
                    );
                }
                return;
